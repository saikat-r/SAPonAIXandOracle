...
if [ $# -lt $minargs ]
then
   echo "Bhosri ke... Parameters is Missing... woh kya tera tau dega ? "
   RC=08
   exit $RC
fi
...
if [[ -z $debugmode ]]; then echo "$( basename $0 )" ; fi
...
echo ""
#Check DB Backup
echo "Check DB Backup Status:"
echo "-----------------------"
echo ""
DBbackupFile=`ls /oracle/$SIDR/sapbackup/*.anf | tail -n 1`
