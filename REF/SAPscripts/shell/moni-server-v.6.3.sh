...
echo "-----------------"
lsps -a > templsps
TOTSWP=`more templsps | grep swapnfs1 | awk '{print $4}'`
USEDSWP=`more templsps | grep swapnfs1 | awk '{print $5}'`
...
# Check saposcol agent
echo ""
echo "Check saposcol agaent:"
echo "----------------------"
ps -ef | grep saposcol | grep host
echo ""

# Check SAP processes
if [ $APP = "CI" -o $APP = "ci" ]; then
echo "SAP Systems on this server: "
ps -ef | grep ms.sap | grep pf | awk '{ print $(NF-1) }' | cut -c 7-9
fi
if [ $APP = "JI" -o $APP = "ji" ]; then
echo "SAP Systems on this server: "
ps -ef | grep jc.sap | grep pf | awk '{ print $(NF-1) }' | cut -c 7-9
fi

# Check SAP instances
if  [ $APP = "CI" -o $APP = "ci" ]; then
echo "SAP instances on this server: "
ps -ef | grep ms.sap | grep pf | awk '{ print $(NF-1) }' | cut -d _ -f 2
fi
if [ $APP = "JI" -o $APP = "ji" ]; then
echo "SAP instances on this server: "
...
# Workprocess table SM50
SAPprofile=`ps -ef | grep ms.sap | grep pf | awk '{ print $(NF) }'`
echo "SAP profile file is : "
echo $SAPprofile
echo ""
echo ""
echo ""
echo "q" | dpmon $SAPprofile p
echo "q" | dpmon $SAPprofile p > /home/users/in10c2/moni/monitoringLOGS/$datesh/dpmon_$APP$SID$SNO

#rm dpmon_$APP$SID$SNO
