...
for /f "tokens=1-18* delims=," %%A in (ALL_PHY_HOSTIP.TXT) do (
  IF [%%~B] == [] (
    echo IP of HOST %%~A not found
      ...
      echo y | plink -ssh -l exin051f -pw Start123 %%~B sh /home/users/in10c2/moni/readSERVICEdataAUTO.sh %DATESTAMP_PARSE% >> LOGS\VLANsheet_%DATESTAMP_PARSE%.CSV
  )
)
copy LOGS\VLANsheet_%DATESTAMP_PARSE%.CSV VLANsheetLIVE.CSV
