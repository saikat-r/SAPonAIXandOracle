...
@echo off
for /f "tokens=1-18* delims=," %%A in (ALL_PROD_PHY_HOSTIP.TXT) do (
  IF [%%~B] == [] (
    echo IP of HOST %%~A not found
    ) ELSE (
      echo Collecting data now for SERVER %%~A with IP=%%~B
      echo y | plink -ssh -l in10c2 -pw Start1234 %%~B sh /home/users/in10c2/moni/readHOSTdata.sh %DATESTAMP_PARSE% >> LOGS\VLANsheet_%DATESTAMP_PARSE%.CSV
  )
)
copy LOGS\VLANsheet_%DATESTAMP_PARSE%.CSV VLANsheet_PROD.CSV
