...
#set -x
msHOST=`ps -ef | grep se.sap | grep pf | awk '{ print $(NF) }' | cut -c 43-`
jcHOST=`ps -ef | grep jc.sap | grep pf | awk '{ print $(NF) }' | cut -c 43-`
NUMmsHOST=`ps -ef | grep se.sap | grep pf | awk '{ print $(NF) }' | cut -c 43- | wc -l`
...
echo "SAP systems running here........" `ps -ef | grep se.sap | grep pf | awk '{ print $(NF-2) }' | cut -c 7-9` `ps -ef | grep jc.sap | grep pf | awk '{ print $(NF-1) }' | cut -c 7-9` ;
...
echo "ABAP Instance Running here......" `ps -ef | grep se.sap | grep pf | awk '{ print $(NF-2) }' | cut -d _ -f 2` ;
echo "Java Instance Running here......" `ps -ef | grep jc.sap | grep pf | awk '{ print $(NF-1) }' | cut -d _ -f 2` ;
echo "ABAP Hostnames available here..." `ps -ef | grep se.sap | grep pf | awk '{ print $(NF) }' | cut -f 3 -d _` ;
echo "JAVA Hostnames available here..." `ps -ef | grep jc.sap | grep pf | awk '{ print $(NF) }' | cut -f 3 -d _` ;
echo "DB running on this server......." `ps -ef | grep tns | awk '{ print $(NF-1) }' | grep LIST | cut -f 2 -d _`
echo ""
